...
function renderPage({ channels, countries, categories, countryNames, total, page, totalPages, search, country, category }) {
  ...
  const cards = pageChannels.length ? pageChannels.map(channel => `
    <div class="col-6 col-sm-4 col-md-3"><div class="card channel-card h-100 p-2" onclick='playStream(...${escapeHtml(JSON.stringify([channel.stream_url, channel.name, channel.id, channel.program_name || '']), true)})'>
      ...
      <div class="text-center text-muted" style="font-size:0.7rem">${countryFlag(channel.country || '')} ${escapeHtml(countryNames[channel.country] || channel.country || 'Unknown')}</div>
    </div></div>`).join('') : '<div class="col-12 text-center text-muted p-5"><h5>No channels found.</h5></div>';
  ...
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Global IPTV</title>
...
function styles() {
  return `:root{--glass:#121920c7;--border:#ffffff33}body{position:relative;min-height:100vh;overflow-x:hidden;background:#1a1e23;color:#fff}.background-animation{position:fixed;inset:0;z-index:0;overflow:hidden;pointer-events:none}.bubble{position:absolute;bottom:-10rem;left:calc(var(--i)*12%);width:6rem;height:6rem;border:1px solid #ffffff2e;border-radius:50%;background:#5ca1be14;animation:float 22s linear infinite}@keyframes float{to{transform:translateY(-125vh);opacity:0}}.container{position:relative;z-index:1}.glass-panel,#player-wrapper,.channel-card{background:var(--glass);border:1px solid var(--border);border-radius:1rem;box-shadow:0 1.5rem 3rem #0003;backdrop-filter:blur(1.25rem)}.header-panel{padding:1.25rem}.glass-control{background:#ffffff17!important;border:1px solid var(--border)!important;color:#f8fafc!important}.glass-control::placeholder{color:#ffffff9e}.glass-control option{background:#202b34}.channel-card{cursor:pointer;transition:.25s}.channel-card:hover{background:#ffffff2b;border-color:#7dd3fcb8;transform:translateY(-.25rem)}.logo-container{height:80px;display:flex;align-items:center;justify-content:center;background:#030000;padding:5px;border-radius:12px}.channel-logo{max-height:100%;max-width:100%;object-fit:contain}#player-wrapper{position:sticky;top:20px;z-index:2;overflow:hidden}video{width:100%;aspect-ratio:16/9;background:#000}.pagination{justify-content:center;flex-wrap:wrap}.page-link{background:#ffffff05;border-color:var(--border);color:#fff}.page-item.active .page-link{background:#38bdf880;border-color:#7dd3fcbf}.text-muted{color:#aaa!important}.player-meta{background:#0000003d!important}@media(max-width:768px){.container{padding-left:1rem;padding-right:1rem}.input-group{flex-wrap:wrap;gap:.5rem}.input-group>*{flex:1 1 100%;border-radius:.375rem!important}}`;
}

function clientScript() {
  return `const video=document.getElementById('video-player'),titleEl=document.getElementById('now-playing-title'),statusEl=document.getElementById('now-playing-status'),programEl=document.getElementById('now-playing-program');let shakaPlayer,pendingStream;function updateProgram(name){programEl.textContent=name?'Now playing: '+name:''}async function loadStream(url){if(!shakaPlayer){pendingStream={url};return}try{await shakaPlayer.load(url);statusEl.textContent='Live';statusEl.className='text-success'}catch(error){console.error(error);statusEl.textContent='Stream unavailable';statusEl.className='text-danger'}}async function init(){const ui=video.ui;if(!ui){statusEl.textContent='Player unavailable';return}shakaPlayer=ui.getControls().getPlayer();shakaPlayer.addEventListener('error',()=>{statusEl.textContent='Stream unavailable';statusEl.className='text-danger'});if(pendingStream){const stream=pendingStream;pendingStream=null;loadStream(stream.url)}}document.addEventListener('shaka-ui-loaded',init);function playStream(url,name,id,program,scroll=true){titleEl.textContent=name;statusEl.textContent='Connecting to stream...';statusEl.className='text-info';updateProgram(program);sessionStorage.setItem('iptv-current-stream',JSON.stringify({url,name,id,program}));if(scroll)window.scrollTo({top:0,behavior:'smooth'});loadStream(url)}document.getElementById('filter-form').addEventListener('submit',event=>{event.preventDefault();const url=new URL(location.href);url.search=new URLSearchParams(new FormData(event.target));url.searchParams.set('page','1');location.href=url});const saved=sessionStorage.getItem('iptv-current-stream');if(saved){try{const stream=JSON.parse(saved);if(stream.url&&stream.name)playStream(stream.url,stream.name,stream.id,stream.program||'',false)}catch(error){sessionStorage.removeItem('iptv-current-stream')}}`;
}
